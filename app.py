...
def build_worksheet_prompt(data: dict) -> tuple[str, str]:
    ...
    system_instruction = (
        'Sen kıdemli bir eğitim teknolojisi uzmanı, ölçme-değerlendirme uzmanı, UX/UI tasarımcısı ve eğitim yazılımı geliştiricisisin.\n\n'
        'İKİ TEMEL YAKLAŞIMI BİRLİKTE VE BÜTÜNLEŞİK KULLAN:\n\n'
        'A — TYMM BAĞLAM TEMELLİ YAKLAŞIM:\n'
        'ÖĞRENME ÇIKTISI → BECERİ → SÜREÇ BİLEŞENİ → BAĞLAM → KANIT/VERİ → AKIL YÜRÜTME → ÇIKARIM/DEĞERLENDİRME/KARAR → SORU.\n'
        '- Bağlam gerçekçi, anlamlı ve öğrenci açısından yaşantısal olmalıdır.\n'
        '- Öğrenci bilgiyi sadece hatırlamamalı; bilgiyi kullanmalı, yorumlamalı, ilişkilendirmeli, çıkarım yapmalı, gerekçelendirmeli veya değerlendirmelidir.\n\n'
        'B — MMR YAKLAŞIMI:\n'
        'BİLGİ → HAYRET → ANLAMLANDIRMA → HİKMET/KUDRET → TEFEKKÜR → DEĞER → SORUMLULUK → HAYATA YANSITMA.\n'
        '- MMR boyutu konuya sonradan eklenen bağımsız bir dinî bölüm OLMAMALIDIR. '
        'TYMM BAĞLAM TEMELLİ YAKLAŞIM kısmında ilgili olan yerleri açıklayan, onları destekleyen hatta onlar ile beraber kullanılan bir yapı olmalıdır.\n'
        '- Bilimsel bilgi ile manevi anlam birbirinden ayrılmamalı; ancak birbirine de karıştırılmamalıdır.\n'
        '- Bilimsel gerçekler doğru aktarılmalı, manevi boyut ise öğrencinin düşünmesini ve anlamlandırmasını sağlayacak şekilde oluşturulmalıdır.\n'
        '- Öğrenciye ASLA hazır sonuç dayatma ("Allah\'ın kudretidir/şükretmeliyiz" deme; açık uçlu sorularla düşündür).\n\n'
        'PROFESYONEL EĞİTİM MATERYALİ UX/UI TASARIM İLKELERİ:\n'
        '1. GÖRSEL HİYERARŞİ: Başlık → Bağlam → Veri → Görev → Soru → Tefekkür → Müzakere → Hayat Bağlantısı → Öz Değerlendirme.\n'
        '2. BÖLÜMLERİN KİMLİĞİ: Her ana bölümün kendine ait başlığı, ikonu, görsel ayracı ve öğrenci etkileşim alanı bulunsun. '
        'Modern, sade, akademik ve MEB materyali ciddiyetinde olsun (çocukça renk cümbüşü yapma).\n'
        ...
        '4. A4 BASKI VE DİJİTAL UYUMLULUK: Çıktı A4 yazdırmaya ve responsive dijital kullanıma tam uyumlu olsun.'
    )

    user_prompt = (
        f'Aşağıda verilen eğitim bilgilerini temel alarak TYMM + MMR BAĞLAM TEMELLİ ÇALIŞMA KÂĞIDI ANA PROMPTU standardında '
        f'eksiksiz, profesyonel, içerik odaklı ve yazdırılabilir bir ÇALIŞMA KAĞIDI hazırla:\n\n'
        ...
        f'ZORUNLU ÜRETİM KURALI:\n'
        f'Çalışma kâğıdını üretmeden önce İLK OLARAK aşağıdaki planlama ve tasarım kararları tablosunu eksiksiz oluştur:\n\n'
        f'### TYMM + MMR ÖĞRETİM TASARIMI VE PLANLAMA MATRİSİ\n'
        f'| Alan | Karar |\n'
        f'| :--- | :--- |\n'
        f'| **Ders** | {data.get("subject")} |\n'
        f'| **Sınıf** | {data.get("grade")} |\n'
        f'| **Konu** | {data.get("topic")} |\n'
        f'| **Öğrenme çıktısı** | {data.get("learning_outcome")} |\n'
        f'| **Beceri** | {skill} |\n'
        f'| **Süreç bileşeni** | {process_comp} |\n'
        f'| **Bağlam** | [Seçilen gerçek yaşam problem durumu/senaryosu] |\n'
        f'| **Kullanılacak veri** | [Tablo, grafik, deney, model veya gözlem seti] |\n'
        f'| **TYMM düşünme süreci** | [Öğrencinin yapacağı akıl yürütme, çözümleme ve çıkarım basamakları] |\n'
        f'| **MMR bağlantısı** | [Ders bilgisinden doğal doğan düzen, ölçü, denge ve hikmet boyutu] |\n'
        f'| **Değer** | [Konunun içinden keşfedilecek ahlaki erdem ve değer] |\n'
        f'| **Tefekkür sorusu** | [Öğrencinin zihnini ve kalbini harekete geçirecek açık uçlu tefekkür sorusu] |\n'
        f'| **Müzakere sorusu** | [Sınıfta tartışılacak gerekçeli akran müzakeresi sorusu] |\n'
        f'| **Hayata yansıtma** | [Öğrencinin günlük hayatına taşıyacağı somut davranış ve sorumluluk] |\n\n'
        f'ARDINDAN ÇALIŞMA KÂĞIDINI AŞAĞIDAKİ 10 BÖLÜMLÜ STANDART İÇERİK SIRASINA GÖRE ÜRET:\n\n'
